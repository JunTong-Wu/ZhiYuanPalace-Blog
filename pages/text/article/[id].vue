      ...
      <zy-fetch-loading :fetchData="articleData">
        ...
        <template v-slot:onload="{ row: item }">
          ...
          <div
            ...
            pt="8 xs:10 sm:12 md:14 lg:16"
            px="8 xs:10 sm:12 md:14 lg:16"
          >
            <h1
              text="2xl xs:2xl sm:3xl md:3xl lg:3xl"
              mb="8 xs:10 sm:12 md:14 lg:16"
              mt-0
            >
              {{ item.article_title }}
            </h1>
...
watch(articleData.pending, (newValue) => {
  if (!newValue) {
    if (articleData.res.value && articleData.res.value.code == 200) {
      setTimeout(() => {
        cardTransitionEnd();
      }, 200);
    }
