<template>
  <ZyFetchLoading
    :fetchData="musicListDataLazyFetch"
    @fetchOnload="showMusicList"
  >
    ...
    <template #onload>
      ...
      >
        <template #cover-data="{ row }">
          <img
            class="h-16 w-16 object-cover rounded-xs"
            :src="`${cdnUrl}${row.cover}`"
            alt=""
            srcset=""
          />
        </template>
        ...
        <template #actions-data="{ row }">
          <div class="flex items-center space-x-4">
            ...
            >
              <UIcon
                v-if="playingId == row.id && musicPlayState"
...
<script setup lang="ts">
  ...
  const musicListData = ref<MusicListModelType>(new music.MusicList());
