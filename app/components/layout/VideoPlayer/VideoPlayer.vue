<template>
  <div>
    <!-- 返回按钮 -->
    <div class="absolute z-10 landscape:top-12 landscape:left-12 flex items-center justify-center">
      <GoBackButton class="dark landscape:bg-black !m-0 landscape:rounded-full landscape:opacity-30" />
    </div>
    <!-- 背景图 -->
    <div class="portrait:hidden absolute w-full h-full">
      <img :src="`${cdnUrl}${videoData.video_cover}`" alt=""
        class="w-full h-full object-cover blur-3xl brightness-75 scale-125">
    </div>
    <!-- 视频播放器 -->
    <div class="absolute w-full h-full pb-20 portrait:pb-8">
      <div class="absolute landscape:hidden w-full h-full flex items-center justify-center transition-all duration-500"
        :class="{
      ...
      <video ref="videoElement" class="w-full h-full" @click="togglePlay">
      </video>
    </div>
    <!-- 视频介绍 -->
    <div class="absolute bottom-20 w-full portrait:bottom-8 bg-gradient-to-b from-transparent to-[rgba(0,0,0,0.4)]">
      <div class="p-12 portrait:p-4">
        <h2 class="text-xl font-bold">@玉淇冰Mebius ·
    ...
    <div
      ...
      <div class="absolute -top-1 left-0 w-full">
        <URange @mousedown.stop @mousemove.stop @mouseup.stop @input="progressBarInput" v-model="progressBar" />
      </div>
    ...
    <div class="absolute right-0 bottom-20 portrait:bottom-48 w-20 py-8">
      <div class="flex flex-col items-center gap-8 portrait:gap-14">
        <ZyButton class="rounded-full p-1 bg-white" type="transparent">
          <img src="@/assets/image/avatar.jpg" alt="" class="w-12 h-12 portrait:w-14 portrait:h-14 rounded-full" />
        </ZyButton>
        <ZyButton class="h-music aspect-square" title="点赞" type="transparent">
          <div class="drop-shadow portrait:drop-shadow-xl">
            <UIcon name="i-solar-heart-bold" class="w-10 h-10 portrait:w-14 portrait:h-14" />
            <span class="block portrait:text-xl drop-shadow -mt-2">{{ videoData.video_likes }}</span>
          </div>
        </ZyButton>
        <ZyButton class="h-music aspect-square" title="评论" type="transparent">
          <div class="drop-shadow portrait:drop-shadow-xl">
            <UIcon name="i-solar-chat-round-dots-bold" class="w-10 h-10 portrait:w-14 portrait:h-14" />
            <span class="block portrait:text-xl drop-shadow -mt-2">{{ videoData.video_likes }}</span>
          </div>
        </ZyButton>
      </div>
