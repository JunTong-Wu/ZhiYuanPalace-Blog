...
<template>
  <UAccordion multiple :items="linkList" :ui="{ wrapper: 'flex flex-col w-full gap-2' }">
    <template #default="{ item, index, open }">
      <div v-if="!item.children" class="rounded-xs hover:bg-background"
        ...
        <ZyLink v-zy-ripple :to="`${item.path}`" :title="$t(`menu.${String(item.name)}`)"
          ...
          <div class="w-full flex items-center gap-2 relative">
            <ZyIcon class="flex-none" size="1.5rem" :defaultName="item.meta?.defaultIcon"
              defaultColor="var(&#45;&#45;text-2)" :activatedName="item.meta?.activatedIcon"
              :activated="isActivateRouter(route, item.path)" activatedColor="var(&#45;&#45;text-1)" />
            <span class="text-sm transition-all line-clamp-1" :class="{
      ...
      <UButton v-else variant="ghost" class="py-4 rounded-xs bg-transparent hover:bg-background text-sm pl-5"
        ...
        <span class="text-start text-sm transition-all line-clamp-1" :class="{
          'opacity-100 w-32': !hide,
          'opacity-0 w-0': hide,
        }">{{ $t(`menu.${String(item.name)}`) }}</span>
        <template #trailing>
      ...
      <ul v-if="item.children" class="flex flex-col w-full gap-2">
        <li v-for="children in item.children" class="rounded-xs h-14 flex hover:bg-background"
          ...
          <ZyLink v-zy-ripple :to="`${children.path}`" :title="$t(`menu.${String(children.name)}`)"
            ...
            <div class="w-full flex items-center gap-2 relative">
              <ZyIcon class="flex-none" size="1.5rem" :defaultName="children.meta?.defaultIcon"
                defaultColor="var(&#45;&#45;text-3)" :activatedName="children.meta?.activatedIcon"
                :activated="isActivateRouter(route, children.path)" activatedColor="var(&#45;&#45;text-2)" />
