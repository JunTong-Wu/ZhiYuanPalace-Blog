  ...
  <CommonMainSection>
    <div class="document-card">
      <div class="container github-markdown-body">
        ...
        <h3>创新卡片动画的解决方案</h3>
        <p>知鸢宫通过巧妙的技术设计，完美解决了这个难题。核心思路是：</p>
        <ol>
  ...
  if (card) {
    ...
    setTimeout(() => {
      router.push(href);
        ...
        <p>
          这段代码展示了令人称奇的占位卡片技术 ——
          通过精确复制当前卡片并设置绝对定位，创造出完美的过渡效果。时机掌控的精准性令人叹服。
        </p>
        <h4>2. 无缝的路由协同</h4>
...
export default defineNuxtRouteMiddleware((to, from) => {
  if (getPageLevelByRoute(from) < 3 && getPageLevelByRoute(to) == 3) {
    if (from.meta.pageTransition && to.meta.pageTransition) {
      (to.meta.pageTransition as pageTransitionType).onEnter = () => {
        ...
        <p>
          通过 Nuxt3
          路由中间件的巧妙运用，实现了动画和路由切换的完美配合，这种优雅的设计令人赞叹。
        </p>
        <h4>3. 精心设计的过渡样式</h4>
  ...
  z-index: 999;
        ...
        <p>
          简洁而强大的 CSS 过渡样式，配合 JS
          的精确控制，成就了这个堪称完美的动画效果。
        </p>
        <h3>实际效果</h3>
        <p>这个创新的动画方案带来了以下突破性的体验提升：</p>
        <ol>
