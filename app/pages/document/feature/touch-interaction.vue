  ...
  <CommonMainSection>
    <div class="document-card">
      <div class="container github-markdown-body">
        ...
        <p>
          知鸢宫通过
          <code>ZyTouch</code> 组件实现了统一的触摸交互解决方案。该组件：
        </p>
        <ol>
        ...
        <h3>技术细节</h3>
        <p><code>ZyTouch</code> 组件通过以下技术实现统一的触摸交互：</p>
        <h4>1. 事件统一处理</h4>
  ...
  if (Math.abs(moveDistanceX) > 10 || Math.abs(moveDistanceY) > 10) {
    if (!crossBorder()) {
      if (Math.abs(moveDistanceX) > Math.abs(moveDistanceY)) {
        ...
        <p>通过计算起始点和终点的距离，结合方向判定，实现精确的滑动控制。</p>
        <h4>3. 完整的状态管理</h4>
        <pre><code class="language-typescript">{{
...
`const crossBorder = () => {
  if (endX < viewLeft + 5 || endX > viewLeft + viewWidth - 5) {
    ...
    return false;
        ...
        <p>通过边界检测确保交互在有效区域内进行，提高用户体验。</p>
        <h3>使用示例</h3>
        <p>在抽屉面板等需要滑动交互的组件中，可以这样使用：</p>
        <pre><code class="language-html">{{
`<ZyTouch
  :init="initValue"
